...
class TauntHandlerTrait extends BossTrait
{
    ...
    function OnFrameTickAlive()
    {
        ...
        else if (tauntId == 1118 || tauntId == 1157 || tauntId == 1162 || tauntId == 1106) //Dance Taunts, Gray Mann responds to these.
        {
            if (!isTaunting)
            {
                EmitPlayerVODelayed(boss, "dance", 3.0);
                isTaunting = true;
            }
        }
        ...
        else if (tauntId == 1107 || tauntId == 1111) //Flippin' Awesome or Skullcracker
        {
            if (time - partnerJoinTime > (tauntId == 1107 ? 1.45 : 1.72))
            {
                ...
                if (tauntPartner != null)
                {
                    ...
                    custom_dmg_hale_taunt.SetAbsOrigin(boss.GetOrigin());
